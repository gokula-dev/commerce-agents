...
"""Maps one raw ``products_en_us`` Typesense document to the flat, family/variant-shaped
dict ``examples/demo_common/storefront_fixtures.load_catalog`` expects on disk (the same
shape ``examples/retail/data/catalog.json`` uses): a family record with a compact
``variants`` list when more than one color/size/style is eligible, or a Plain record under
the single eligible variant's own id when there is exactly one (``docs/backends.md`` Step 4).

Eligibility, per variant, mirrors Cortex V4's own product-lookup adapter filter (not just
the top-level ``hideInSearch``/``physicalProduct`` filter already applied at export time):
...
stock" — the raw data has no separate live-inventory signal, only this sellability window,
so a product with zero eligible variants is left out of the catalog entirely rather than
having stock fabricated for it. State this plainly in any write-up: ``in_stock`` here means
"currently in its sellable window", not "in stock at a warehouse right now".
"""
...
def _flatten_specs(node: Any, out: dict[str, str], limit: int = _MAX_SPECS) -> None:
    ...
    ``specs``/``facet`` objects with no fixed depth) into a flat label -> value dict. Noise
    tolerant: a facet with no value or numeric is a category header, not a leaf spec, and
    is only descended into, never recorded."""
    if len(out) >= limit:
        return
...
def _variant_record(
    variant: dict[str, Any], *, family_id: str, option_field: str, index: int
) -> dict[str, Any]:
    record: dict[str, Any] = {
    ...
    if option_field in _OPTION_FIELDS and variant.get(option_field):
        ...
        # No declared facet actually varies across these variants (rare) — fall back to a
        # positional option so the family still models as options rather than colliding ids.
...
def normalize_document(doc: dict[str, Any], *, now_ms: int | None = None) -> dict[str, Any] | None:
    """One catalog.json entry (Plain or Family), or ``None`` when nothing in it is
    currently eligible."""
    ...
    if len(variants) == 1:
        # Plain: served under the one eligible variant's own id, per docs/backends.md's
        # "product shell that always has variants" mapping row.
        variant = variants[0]
        plain: dict[str, Any] = {
    ...
    family: dict[str, Any] = {
        ...
        "specs": specs,
        "variants": [
            _variant_record(v, family_id=family_id, option_field=option_field, index=i)
