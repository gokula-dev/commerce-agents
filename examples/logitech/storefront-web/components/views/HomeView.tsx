...
const STARTERS: Starter[] = [
  { icon: "search", prompt: "A quiet wireless mouse for the office, under $50" },
  { icon: "home", prompt: "Set up a video-conferencing kit for a small home office" },
  { icon: "tag", prompt: "Mechanical or membrane keyboard for long typing days?" },
  { icon: "edit", prompt: "Remember: I prefer compact, ergonomic peripherals in black" },
];
...
function Brief({ orders }: { orders: Order[] | null }) {
  if (!orders) return <>Ask about a product or a project.</>;
  const open = upcoming(orders);
  if (!open.length) return <>Ask about a product or a project.</>;
